"""
run_canonical_fits.py — Producer for sparc_T2-T9_canonical_fits.csv

Re-runs the canonical Burkert / NFW / framework (0/1/2 shell) fits across the
102-galaxy SPARC T=2-9 sample and produces the canonical fits CSV.

This is the corrected-constraint version: shell width is enforced as
sigma/r_shell ≤ 0.4 (matching the manuscript's stated methodology), rather
than the looser sigma ≤ r_max × 0.4 that was implemented in earlier code.
...
Usage:
  ...
  Then:
    python3 run_canonical_fits.py
  
  Output:
    - sparc_T2-T9_canonical_fits.csv    (102 rows × 43 columns)
    - run_canonical_fits.log            (per-galaxy fit summary)

  Expected runtime: 30-90 minutes on a modern Mac (Apple Silicon ~30 min,
  Intel ~60-90 min). Multi-restart with full grid for stability.

Conventions (matching dc14_universal_failures.py and verified against the
shipped CSV's internal relationships):

  Mass-to-light:     Upsilon_disk = 0.5, Upsilon_bulge = 0.7
  Sigma_V floor:     1.0 km/s (minimum observational uncertainty)
  Exclusion rule:    drop points where V_obs^2 ≤ V_bar^2
  Sample filter:     T ∈ [2, 9]
  BIC formula:       chi^2 + k·ln(n_pts_used), k = 2/2/5/8 for Burk/NFW/FW1/FW2
  chi^2_red formula: chi^2 / max(n_pts_used - k, 1)

Constraints (the corrected version — strictly enforced via re-parameterization):

  Burkert:   rho_0 ∈ [1e3, 1e10] M_sun/kpc^3, a ∈ [0.1, 200] kpc
  NFW:       rho_s ∈ [1e3, 1e10] M_sun/kpc^3, r_s ∈ [0.1, 500] kpc
  Framework: per-shell M ∈ [1e6, 5e10] M_sun
             per-shell r_sh ∈ [0.2, R_SH_MAX] kpc, where R_SH_MAX is the
                 multi-restart upper bound (one of 3, 6, or 12 kpc)
             per-shell sigma = sigma_frac × r_sh, with sigma_frac ∈ [0.01, 0.4]
             This re-parameterization HARD-ENFORCES sigma/r_shell ≤ 0.4 via
             curve_fit's box bounds, with no soft penalty.
...
OUTPUT_CSV = './sparc_T2-T9_canonical_fits.csv'
LOG_FILE = './run_canonical_fits.log'
