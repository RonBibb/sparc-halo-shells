...
The framework adequately fits 91/102 galaxies, compared with 65/102 for Burkert-only and 52/102 for free-concentration NFW. Shell-bearing fraction declines systematically with morphological type (per-galaxy T-label permutation $p_{\rm two\mbox{-}sided}=0.002$; per-bin Spearman $\rho=-0.833$, $p=0.010$; Mann-Kendall $p=0.003$), driven by a strong excess in early-type spirals and low shell fractions in late types. Synthetic null tests show that smooth-Burkert and smooth-NFW truth cannot reproduce the observed morphology gradient: Burkert-truth false-positive rates remain uniformly low and fall far below real-data shell-bearing fractions in every T-bin, while NFW-truth produces high but non-monotonic shell-selection rates inconsistent with the observed trend.

Replacing the Burkert backbone with the more flexible Einasto profile preserves both primary findings: the morphology gradient and the requirement for localized structure in NGC~5055, where Burkert, NFW, DC14, and Einasto smooth profiles all fail to reproduce the observed rotation-curve structure.
...
The proper test is to re-run the framework with $\Upsilon_{\rm disk}$ values varying by T-type per the \citet{Schombert2022} color calibrations (e.g., $\Upsilon_{\rm disk}(T=2) \approx 0.65$, $\Upsilon_{\rm disk}(T=5) \approx 0.5$, $\Upsilon_{\rm disk}(T=9) \approx 0.40$) and assess whether shell-bearing fractions converge across T-bins. We do not perform that re-fit here. The \citet{Lelli2016} sensitivity analysis (which tested $\Upsilon_{\rm disk} \in [0.4, 0.8]$ and found the SPARC scaling relations qualitatively robust) is consistent with persistence of the qualitative findings under modest $\Upsilon$ variation, but the specific question of whether the morphology gradient's amplitude is partly $\Upsilon$-driven cannot be settled from the present analysis alone. We therefore flag this as the highest-priority follow-up systematic test, alongside the generalized-NFW and free-shape backbone variants noted in \S\ref{sec:base-profile}.
